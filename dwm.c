 ...
 drawbar(Monitor *m)
 {
	int x, w;
 	...
 	if (m == selmon) { /* status is only drawn on selected monitor */
                ...
                x = wbar - wstext;
                drw_rect(drw, x, 0, LSPAD*2, bh, 1, 1); x += LSPAD*2;  /* to keep left padding clean note: add'*2' by jacob to clean the right padding, you can adjust the value of x here to adjust the text draw position, then serve the 'same' func of the following draw_rect func (the drw_rect leaving blank, not good) */
                for (;;) {
                ...
                drw_setscheme(drw, scheme[SchemeNorm]);
              /*  drw_rect(drw, x, 0, wbar - x, bh, 1, 1);  to keep right padding clean note by jacob: change the value of x when drawing, also do the clean padding job */
